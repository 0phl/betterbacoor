...
export function renderOfflineEmergency(data, logo) {
  ...
  const sources = ids =>
    `<p class="sources">Sources: ${ids
      .map(id => {
        ...
        return `<a href="${escape(source.url)}">${escape(source.name)}</a>`;
      })
      .join(' · ')}</p>`;
  return `<!doctype html>
...
<p>Information snapshot: ${escape(data.last_verified)}. This is not a live advisory feed. Review this information within ${data.review_interval_days} days of the snapshot date.</p>
...
${data.situations.map(item => `<details id="${escape(item.id)}" open><summary>${escape(item.label)} · ${escape(item.title)}</summary><ol>${item.steps.map(step => `<li><strong>${escape(step.title)}</strong><p>${escape(step.text)}</p></li>`).join('')}</ol>${sources(item.sources)}</details>`).join('')}</section>
